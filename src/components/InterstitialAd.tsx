...
export function InterstitialAd({ locale, onClose }: InterstitialAdProps) {
      ...
      >
        ...
        <div className="bg-gradient-to-br from-indigo-900 to-cyan-900 p-8 text-center">
          ...
          <p className="mt-2 text-sm text-cyan-100/80">
            Now with 300% more Sisu per sip. Doctors hate this trick!
          </p>
          <button
            type="button"
            className="mt-6 inline-flex items-center gap-2 rounded-full bg-cyan-500 px-6 py-2.5 text-sm font-bold text-slate-900"
          >
            Install now
        ...
        <div className="p-3">
          <button
            type="button"
            onClick={onClose}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-700 py-3 text-sm font-semibold text-white transition hover:bg-slate-600"
          >
            <X className="h-4 w-4" />
            {ui.adSkip} (3s demo)
          </button>
